	...
	function PK_FX_PRESETS () {
		...
		app.listenFor ('RequestFXUI_Gain', function () {
			...
			var getvalue = function ( q ) {
				...
				if (auto) {
					...
					var db = clamp ( input.value / 1, -60, 24 );
					value = [{val: dbToGain ( db )}];
				}
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row" style="border:none"><label>Gain</label>' + 
				'<input type="range" class="pk_horiz" min="-60.0" max="24.0" step="0.1" value="0.0" />'+
			  ...
			  setup: function( q ) { app.fireEvent ("RequestPause");
				  ...
				  range.oninput = function() {
					var db = clamp ( range.value / 1, -60, 24 );
					span.innerHTML = formatDb ( db );
					app.fireEvent ('RequestActionFX_UPDATE_PREVIEW', [{val: dbToGain ( db )}]);
				  };
		...
		app.listenFor ('RequestActionFXUI_Normalize', function () {
			...
			var x = new PKSimpleModal({
			  ...
			  buttons:[
				{
					...
					callback: function( q ) {
						var input = q.el_body.getElementsByClassName('pk_horiz')[0];
						var value = dbToGain ( clamp ( input.value / 1, -24, 0 ) );
			  ...
			  body:'<div class="pk_row">'+
				...
				'<input type="range" min="-24.0" max="0.0" class="pk_horiz" step="0.1" value="0.0" />'+
				'<span class="pk_val">0.0 dB</span></div>',
			  setup: function( q ) { app.fireEvent ("RequestPause");
				  var range = q.el_body.getElementsByClassName('pk_horiz')[0];
				  var span = q.el_body.getElementsByClassName('pk_val')[0];
				  ...
				  range.oninput = function() {
					span.innerHTML = formatDb ( clamp ( range.value / 1, -24, 0 ) );
				  };
		...
		app.listenFor ('RequestShowFreqAn', function ( url, args_arr ) {
			...
			var setEvents = function ( obj, _url ) {
				...
				var getAnalyzerPageUrl = function ( analyzer_url, as_iframe ) {
					var analyzer_href = new URL ('pages/' + analyzer_url + '.html', document.baseURI);
					if (as_iframe) {
						analyzer_href.searchParams.set ('iframe', '1');
					}

					return analyzer_href.toString ();
				};
				...
				if (!type)
				{
					...
					if (dat && dat[0]) {
						...
						var wnd = window.open (getAnalyzerPageUrl (url, false), title, "directories=no,titlebar=no,toolbar=no,"+
			...
			else if (type === 1)
			{
				...
				iframe.onload = function (e) {
					...
					iframe.src = getAnalyzerPageUrl (url, true);
				}
			// ---
		...
		function getMultitrackPanelConfig ( panel_name ) {
		...
		function getMultitrackPanelPageUrl ( panel_name, as_iframe ) {
			var panel_href = new URL ('pages/multitrack-panel.html', document.baseURI);
			panel_href.searchParams.set ('panel', panel_name);
			if (as_iframe) {
				panel_href.searchParams.set ('iframe', '1');
			}

			return panel_href.toString ();
		}
		...
		function syncDockBarHeight () {
			...
			for (var i = 0; i < ch.length; ++i) {
				...
				next_height = Math.max (next_height, (ch[i].getAttribute('data-dock-height') || 130) / 1);
			}

			if (next_height > 0) app.ui.BarBtm.Show ( next_height );
			else app.ui.BarBtm.Hide ();
		}

		app.listenFor ('WillUnload', function () {
			var cur_mt;
			for (var panel_key in mt_win) {
				cur_mt = mt_win[panel_key];
				if (cur_mt && !cur_mt.type) {
					cur_mt.destroy && cur_mt.destroy ();
				}
			}
		...
		app.listenFor ('RequestDragMultitrackPanel', function ( panel_name ) {
			if (app.isMobile) {
				alert ('unsupported on mobile');
				return ;
			}

			var cur_win = mt_win[panel_name];
			...
			el_back.onmouseleave = function ( e ) {
				el_back.onmouseup ( e );
				app.fireEvent ('RequestShowMultitrackPanel', panel_name, [ [(window.screenLeft + e.pageX)||0, (window.screenTop + e.pageY)||0], 0]);
			};
		});

		app.listenFor ('RequestShowMultitrackPanel', function ( panel_name, args_arr ) {
			if (app.isMobile) {
				alert ('Currently unsupported on mobile');
				return ;
			}
			...
			var toggle = args_arr[ 0 ];
			var type = args_arr[ 1 ];
			var cfg = getMultitrackPanelConfig ( panel_name );
			var curr_win = mt_win[ panel_name ];
			var toggled = false;

			function closeMultitrackPanel () {
				if (!curr_win) return false;

				if (curr_win.win && curr_win.win.destroy) curr_win.win.destroy ();
				else curr_win.destroy && curr_win.destroy ();

				if (mt_win[ panel_name ] === curr_win) {
					mt_win[ panel_name ] = null;
				}
				curr_win = null;
				return true;
			}
			...
			if (curr_win && toggle)
			{
				...
				closeMultitrackPanel ();

				if (ext) {
					app.fireEvent ('DidToggleMultitrackPanel', panel_name, null);
					return ;
				}
				toggled = true;
			}

			var panel_update = function () {
				curr_win && curr_win.win && curr_win.win.update && curr_win.win.update ();
			};

			var setEvents = function ( obj, _panel_name ) {
				obj.win.destroy = function () {
					app.stopListeningFor ('DidMultitrackChange', panel_update);
					app.stopListeningFor ('DidMultitrackPlaybackState', panel_update);
					app.fireEvent ('DidToggleMultitrackPanel', _panel_name, null);

					if (obj && obj === mt_win[_panel_name]) {
						mt_win[_panel_name] = null;
					}
				};

				app.listenFor ('DidMultitrackChange', panel_update);
				app.listenFor ('DidMultitrackPlaybackState', panel_update);
				app.fireEvent ('DidToggleMultitrackPanel', _panel_name, curr_win);
			};
			...
			if (!type)
			{
				var makePopup = function ( dat ) {
					var extra = '';
					if (dat && dat[0]) {
						dat[0] = Math.max (0, dat[0] - (cfg.popupWidth / 3)) >> 0;
						dat[1] = Math.max (0, dat[1]) >> 0;
						extra = ',left=' + dat[0] + ',top=' + dat[1];
					}

					var wnd = window.open (
						getMultitrackPanelPageUrl (panel_name, false),
						cfg.title,
						'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=yes,width=' + cfg.popupWidth + ',height=' + cfg.popupHeight + extra
					);
					...
					curr_win = mt_win[panel_name];
					setEvents ( curr_win, panel_name );
				};
			...
			{
				var iframe = document.createElement ('iframe');
				iframe.className = 'pk_frqan pk_auxpanel pk_auxpanel_' + panel_name;
				iframe.id = 'pk_mt_' + panel_name;
				iframe.setAttribute ('data-dock-height', cfg.dockHeight);
				iframe.style.width = cfg.dockWidth + 'px';
				iframe.style.height = cfg.dockHeight + 'px';
				...
				iframe.onload = function () {
					if (curr_win && curr_win.type === type)
					{
						curr_win.win = iframe.contentWindow;
						setEvents ( curr_win, panel_name );
						curr_win.win.update && curr_win.win.update ();
					}
				};
				iframe.src = getMultitrackPanelPageUrl (panel_name, true);
			}
		});

		app.listenFor ('RequestDockMultitrackPanel', function ( panel_name, dock_target ) {
			if (!app.multitrack || !app.multitrack.setDockPanel) return ;
			...
			app.multitrack.setDockPanel ( panel_name, dock_target || 'right' );
		});
		...
		// ----
                ...
                app.listenFor ("RequestActionFXUI_ClickRemoval", function () { 
                        var sampleRate = app.engine.wavesurfer.backend.buffer.sampleRate;
                        var x = new PKAudioFXModal({ 
                                id: "clickremoval", 
                                title: "Click Detection Settings", 
                                preview: function ( q ) { 
                                        var inputs = q.el_body.getElementsByTagName("input");
                                        var method = q.el_body.getElementsByTagName("select")[0].value;
                                        app.fireEvent ("RequestActionFX_PREVIEW_ClickRemoval", { 
                                            threshold: inputs[0].value / 100,
                                            maxWidth: inputs[1].value / 1,
                                            method: method
                                        }); 
                                }, 
                                ondestroy: function ( q ) {
                                        app.stopListeningFor ('DidDetectClicks', q._clickDetectCount);
                                        app.stopListeningFor ('DidDetectClicksData', q._clickDetectData);
                                        app.ui.InteractionHandler.on = false;
                                        app.ui.KeyHandler.removeCallback (modal_esc_key);
                                },
                                buttons: [
                                    { 
                                        ...
                                        callback: function( q ) { 
                                            q._detectedClicks = [];
                                            q._detectedSelection = null;
                                            var inputs = q.el_body.getElementsByTagName("input");
                                            app.fireEvent ("RequestAction_DetectClicks", { 
                                                threshold: inputs[0].value/100,
                                                maxWidth: inputs[1].value/1
                                            }); 
                                            // We sluiten de modal niet, zodat men daarna op 'Remove' kan klikken
                                        } 
                                    },
                                    { 
                                        ...
                                        callback: function( q ) { 
                                            var method = q.el_body.getElementsByTagName("select")[0].value;
                                            app.fireEvent ("RequestAction_RemoveClicks", { 
                                                method: method,
                                                clicks: q._detectedClicks ? q._detectedClicks.slice() : [],
                                                selection: q._detectedSelection
                                            }); 
                                            q.Destroy (); 
                                        } 
                                    }
                                ], 
                                body: "<div style=\"padding:10px; color:#fff; font-size:13px;\">" +
                                      "<div style=\"margin-bottom:15px; opacity:0.8;\">Configure click detection parameters:</div>" +
                                      "<div class=\"pk_row\"><label style=\"width:120px; display:inline-block;\">Sensitivity</label>" +
                                      "<input type=\"range\" min=\"1\" max=\"100\" step=\"1\" value=\"60\" style=\"width:180px; vertical-align:middle;\" />" +
                                      "<span class=\"pk_val\" style=\"display:inline-block; width:60px; text-align:right; margin-left:10px; border:1px solid #446677; padding:2px 5px; background:#112233;\">60 %</span></div>" +
                                      
                                      "<div class=\"pk_row\"><label style=\"width:120px; display:inline-block;\">Max Click Width</label>" +
                                      "<input type=\"range\" min=\"1\" max=\"2000\" step=\"1\" value=\"500\" style=\"width:180px; vertical-align:middle;\" />" +
                                      "<span class=\"pk_val\" style=\"display:inline-block; width:60px; text-align:right; margin-left:10px; border:1px solid #446677; padding:2px 5px; background:#112233;\">500 smp</span></div>" +
                                      
                                      "<div class=\"pk_row\"><label style=\"width:120px; display:inline-block;\">Removal Method</label>" +
                                      "<select style=\"width:190px; background:#112233; color:#fff; border:1px solid #446677; padding:2px;\">" +
                                      "<option value=\"auto\">Automatic</option>" +
                                      "<option value=\"spline\">Spline Interpolation</option>" +
                                      "<option value=\"linear\">Linear</option>" +
                                      "</select></div>" +
                                      
                                      "<div style=\"margin-top:15px; font-size:11px; opacity:0.7;\" id=\"pk_click_info\">Approx. width at current sample rate: " + (500 / sampleRate * 1000).toFixed(2) + " ms</div>" +
                                      "<div style=\"margin-top:10px; font-size:14px; color:#99FF00; font-weight:bold; border-top:1px solid #334455; padding-top:10px;\" id=\"pk_click_count\">Clicks detected: 0</div>" +
                                      "</div>", 
                                setup: function( q ) {  
                                    app.fireEvent ("RequestPause"); 
                                    var inputs = q.el_body.getElementsByTagName("input"); 
                                    var methodSelect = q.el_body.getElementsByTagName("select")[0];
                                    var info = q.el_body.querySelector("#pk_click_info");
                                    var countDisplay = q.el_body.querySelector("#pk_click_count");
                                    var emitPreviewUpdate = function () {
                                        app.fireEvent("RequestActionFX_UPDATE_PREVIEW", {
                                            threshold: inputs[0].value / 100,
                                            maxWidth: inputs[1].value / 1,
                                    ...
                                    q._clickDetectCount = function( count ) {
                                        if (countDisplay) countDisplay.innerHTML = "Clicks detected: " + count;
                                    };
                                    app.listenFor ('DidDetectClicks', q._clickDetectCount);
                                    q._clickDetectData = function( clicks, selection ) {
                                        q._detectedClicks = clicks ? clicks.slice() : [];
                                        q._detectedSelection = selection || null;
                                    };
                                    app.listenFor ('DidDetectClicksData', q._clickDetectData);
                                    ...
                                    inputs[1].oninput = function() { 
                                        this.nextSibling.innerHTML = this.value + " smp"; 
                                        info.innerHTML = "Approx. width at current sample rate: " + (this.value / sampleRate * 1000).toFixed(2) + " ms";
                                        emitPreviewUpdate (); 
                                    };
