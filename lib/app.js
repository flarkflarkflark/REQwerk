(function ( w, d ) {
	...
	function PKAE () {
		...
		q.init = function ( el_id ) {
			...
			q.multitrackEngine = q._deps.multitrackEngine ? new q._deps.multitrackEngine ( q ) : null;
			q.multitrackUI = q._deps.multitrackUI ? new q._deps.multitrackUI ( q ) : null;
			...
			q._loadProjectFromDesktopPath = function(filePath, fileName) {
				...
				q._streamProjectFile(filePath, null).then(function(meta) {
					...
					return q._streamProjectFile(filePath, {
						...
						q.engine.currentFileName = fileName || meta.name || 'project.recwerk';
