		...
		var FXBank = {
				...
				return {
					filter : function ( audio_ctx, destination, source, duration ) {
						...
						var sensitivity = val.threshold || 0.5; // We hergebruiken 'threshold' als gevoeligheid
						var newBuffer = audio_ctx.createBuffer(buffer.numberOfChannels, buffer.length, buffer.sampleRate);
						
						// Calibratie constanten gebaseerd op AudioRestorationVST
						var maxThreshold = 0.350;
						var minThreshold = 0.0002;
						...
						for (var c = 0; c < buffer.numberOfChannels; c++) {
							...
							// 1. Bereken RMS voor adaptieve drempel
							var sum = 0;
							for (var i = 0; i < input.length; i++) sum += input[i] * input[i];
							var rms = Math.sqrt(sum / input.length);
							
							// 2. Bereken drempelwaarde (niet-lineair zoals in VST)
							var curve = Math.pow(1.0 - sensitivity, 2.0);
							var baseThreshold = minThreshold + (maxThreshold - minThreshold) * curve;
							var levelFactor = Math.max(0.02, Math.min(1.5, rms * 1.5 + 0.05));
							var adaptiveThreshold = baseThreshold * levelFactor;

							// 3. Scan voor clicks met tweede afgeleide (acceleratie)
							for (var i = 4; i < input.length - 4; i++) {
								var secondDeriv = Math.abs(input[i] - 2.0 * input[i-1] + input[i-2]);
								
								if (secondDeriv > adaptiveThreshold) {
									// Gevonden! Pas Catmull-Rom interpolatie toe
									var start = Math.max(0, i - 4);
									var end = Math.min(input.length - 1, i + 4);
									
									// Controlepunten voor spline
									var p0 = input[Math.max(0, start - 2)];
									var p1 = input[start];
									var p2 = input[end];
									var p3 = input[Math.min(input.length - 1, end + 2)];
									
									for (var j = start; j <= end; j++) {
										var t = (j - start) / (end - start);
										var t2 = t * t;
										var t3 = t2 * t;
										...
										output[j] = v0 * t3 + v1 * t2 + v2 * t + v3;
									}
									i = end + 1; // Overslaan om her-detectie te voorkomen
								}
							}
						}
