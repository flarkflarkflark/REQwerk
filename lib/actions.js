(function( PKAE ) {
	'use strict';
		...
		function OverwriteBufferWithSegment (_offset, _duration, withBuffer ) {
			...
			var offsetSamples = Math.max(0, ((_offset / 1) * sampleRate) >> 0);
			var durationSamples = Math.max(0, ((_duration / 1) * sampleRate) >> 0);

			// Replacing the entire file with a different-length render should bypass
			// trim/insert, because AudioBuffer cannot represent a zero-frame remainder.
			if (offsetSamples <= 0 && durationSamples >= originalBuffer.length) {
				OverwriteBuffer(withBuffer);
				return [0, withBuffer.length / withBuffer.sampleRate];
			}
		...
		function teardownAudioNode ( node ) {
			if (!node) return ;

			if (node.length > 0)
			{
				for (var i = 0; i < node.length; ++i) {
					teardownAudioNode ( node[ i ] );
				}
				return ;
			}
		...
		function readAutomationValue ( value, fallback ) {
			if (value && value.length)
			{
				return readAutomationValue ( value[ value.length - 1 ], fallback );
			}
		...
		function stopPreview (_fx) {
			...
			{
				teardownAudioNode ( this.PreviewFilter );
			}
			...
			teardownAudioNode ( this.PreviewSource );
...
//		}
		...
		function DetectClicks ( _offset, _duration, val ) {
			var fx_buffer = CopyBufferSegment ( _offset, _duration );
			var fx = FXBank.ClickRemoval ( val );
			var clicks = fx.detect ({ buffer: fx_buffer });
			
			// Converteer sample offsets naar seconden relatief aan het begin van de track
			var sampleRate = fx_buffer.sampleRate;
			return clicks.map(function(click) {
				return {
					start: _offset + (click.start / sampleRate),
					end: _offset + (click.end / sampleRate),
					channel: click.channel
				};
			});
		}

		function RemoveClicks ( _offset, _duration, val, clicks ) {
			// Converteer seconden terug naar sample offsets relatief aan het ge-kopieerde segment
			var sampleRate = wavesurfer.backend.buffer.sampleRate;
			var internalClicks = clicks.map(function(click) {
				return {
					start: Math.round((click.start - _offset) * sampleRate),
					end: Math.round((click.end - _offset) * sampleRate),
					channel: click.channel
				};
			});
			
			var fxParams = Object.assign({}, val, { clicks: internalClicks });
			applyEffect( _offset, _duration, FXBank.ClickRemoval( fxParams ) );
		}
		...
		function DownloadFile( with_name, format, kbps, selection, stereo, callback ) {
			...
			if (format === 'mp3') {
				worker = new Worker('lib/lame.js');
			}
			else if (format === 'flac') {
				worker = new Worker('lib/flac.js');
			}
			else {
				worker = new Worker('lib/wav.js');
			}
			...
			function forceDownload ( blob ) {
				var export_name = with_name ? with_name : 'RECwerk-output.mp3';
				var export_tags = master.multitrack && master.multitrack.getExportMetadata &&
					master.multitrack.getExportMetadata(master.engine.currentFileName, master.engine.currentFilePath);
				var finalize = function ( final_blob ) {
					var url = (window.URL || window.webkitURL).createObjectURL(final_blob);
					...
					if (master.multitrack && export_tags && window.PKMetadataWriter && window.PKMetadataWriter.hasTags(export_tags)) {
						master.multitrack.noteExport(export_name, format, export_tags);
					}

					callback && callback ('done');
				};

				if (window.PKMetadataWriter && export_tags && window.PKMetadataWriter.hasTags(export_tags)) {
					window.PKMetadataWriter.applyMetadataToBlob(blob, format, export_tags)
						.then(finalize)
						.catch(function () {
							finalize(blob);
						});
		...
		function updatePreview ( val ) {
			if (!this.previewing) return ;
			this.PreviewUpdate && this.PreviewUpdate.call ( this, this.PreviewFilter, audio_ctx, val, this.PreviewSource );
		}
		...
		var FXBank = {
			ClickRemoval : function ( val ) {
				return {
					detect : function ( source ) {
						...
						var sensitivity = val.threshold || 0.6;
						var maxWidth = val.maxWidth || 500;
						var clicks = [];
						var sampleRate = buffer.sampleRate;
						
						// Drempels gebaseerd op onderzoek (2-8kHz focus voor crackle)
						var maxThreshold = 0.700;
						var minThreshold = 0.030;
						
						// Voorbereiding voor stereo correlatie check
						var isStereo = buffer.numberOfChannels === 2;
						var left = buffer.getChannelData(0);
						var right = isStereo ? buffer.getChannelData(1) : null;

						for (var c = 0; c < buffer.numberOfChannels; c++) {
							var input = buffer.getChannelData(c);
							var other = (isStereo && c === 0) ? right : (isStereo ? left : null);
							
							// Local noise floor estimation (sliding window)
							var windowSize = Math.floor(sampleRate * 0.02); // 20ms window
							var localRms = 0;
							
							var curve = Math.pow(1.0 - sensitivity, 1.5);
							var baseThreshold = minThreshold + (maxThreshold - minThreshold) * curve;

							for (var i = windowSize; i < input.length - windowSize; i++) {
								// Update local RMS elke 10ms voor performance
								if (i % (windowSize / 2) === 0) {
									var sum = 0;
									for (var s = i - windowSize; s < i; s++) sum += input[s] * input[s];
									localRms = Math.sqrt(sum / windowSize);
								}

								// Second derivative (transient acceleration)
								var d2 = Math.abs(input[i] - 2.0 * input[i-1] + input[i-2]);
								var adaptiveThreshold = baseThreshold * (localRms * 1.2 + 0.05);

								if (d2 > adaptiveThreshold) {
									// MAD-like check voor isolatie
									var isIsolated = true;
									var lookahead = 15;
									var neighborSum = 0;
									for (var m = 1; m <= 5; m++) neighborSum += Math.abs(input[i] - input[i+m]);
									var avgDiff = neighborSum / 5;

									// Insights: Click is transient (kort) en wijkt sterk af van buren
									if (avgDiff > (localRms * 0.2 + 0.03)) {
										
										// Stereo Subtraction Insight:
										// Als het in het andere kanaal op exact dezelfde plek NIET zit, 
										// is de kans dat het een vinyl-tik is 99% (voor gecentreerde muziek).
										if (isStereo && other) {
											var otherD2 = Math.abs(other[i] - 2.0 * other[i-1] + other[i-2]);
											// Als de tik in het andere kanaal veel zwakker is, verhogen we de gevoeligheid
											if (otherD2 < d2 * 0.5) {
												// Bevestigd als asymmetrische tik
											} else if (otherD2 > d2 * 0.8 && otherD2 < d2 * 1.2) {
												// Tik zit in beide kanalen gelijk -> mogelijk muziek transient?
												// We zijn hier voorzichtiger
												if (d2 < adaptiveThreshold * 1.5) isIsolated = false;
											}
										}
										...
										if (isIsolated) {
											...
											// Breedte detectie
											while (end < input.length - 2 && end - start < maxWidth) {
												var nextD2 = Math.abs(input[end] - 2.0 * input[end-1] + input[end-2]);
												if (nextD2 < adaptiveThreshold * 0.5) break;
												end++;
											}
					...
					filter : function ( audio_ctx, destination, source, duration ) {
						var buffer = source.buffer;
						var method = val.method || 'auto';
						var clicks = val.clicks || this.detect(source);
						...
						for (var c = 0; c < buffer.numberOfChannels; c++) {
							...
							var channelClicks = clicks.filter(function(cl) { return cl.channel === c; });
							
							for (var k = 0; k < channelClicks.length; k++) {
								var click = channelClicks[k];
								var start = click.start;
								var end = click.end;

								var p0 = input[Math.max(0, start-1)];
								var p1 = input[start];
								var p2 = input[Math.min(input.length-1, end)];
								var p3 = input[Math.min(input.length-1, end+1)];
						...
						// Mute de originele bron om dubbel geluid of clicks te voorkomen
						var mute = audio_ctx.createGain();
						mute.gain.value = 0;
						source.connect(mute);
						...
						update : function ( filtered_source, audio_ctx, val, source ) {
							if (filtered_source && filtered_source !== source)
							{
								teardownAudioNode ( filtered_source );
							}

							var ff = this.FXBank.ClickRemoval( val );
							this.PreviewFilter = ff.filter ( audio_ctx, audio_destination, source, 0 );
							this.PreviewUpdate = ff.update;
						}
					};
				},
			...
			Bitcrusher : function ( val ) {
							return {
								filter : function ( audio_ctx, destination, source, duration ) {
									var bufferSize = 4096;
									var channels = Math.max (1, source.buffer.numberOfChannels);
									var node = audio_ctx.createScriptProcessor(bufferSize, channels, channels);
									node._bits = Math.max (1, Math.min (16, (val.bits || 8) >> 0));
									
									node.onaudioprocess = function(e) {
										var norm = Math.pow(2, node._bits);
						...
						Filter : function ( val ) {
							return {
								...
								update : function ( filter, audio_ctx, val ) {
									...
									filter.Q.setTargetAtTime(val.q || 1, audio_ctx.currentTime, 0.05);
								}
							};
						},
				...
				return {
					filter : function ( audio_ctx, destination, source, duration ) {
						...
						for (var i = 0; i < source.buffer.numberOfChannels; ++i) {
							...
							if (!equally) {
								if (chan_peak <= 0) continue;

								var diff = max_val / chan_peak;
								for (var k = 0, len = chan_data.length; k < len; ++k) {
									chan_data[ k ] *= diff;
								}
							}
							else if (max_peak < chan_peak)
							{
								max_peak = chan_peak;
							}
						}
			...
			HardLimit : function ( val ) { //todo ASM JS??
				return {
					filter : function ( audio_ctx, destination, source, duration ) {
						...
						look_ahead = Math.max (1, (look_ahead * buffer.sampleRate/1000) >> 0);
						max_val = Math.max (0, Math.min (1, max_val));
						...
						for (var i = 0; i < buffer.numberOfChannels; ++i) {
							...
							for (var b = 0, len = chan_data.length; b < len; b += look_ahead)
							{
								var wnd_end = Math.min ( len, b + look_ahead );
								var max_peak = 0;

								for (var k = b; k < wnd_end; ++k) {
									var curr = Math.abs ( chan_data [ k ] );
									if (max_peak < curr)
										max_peak = curr;
								}

								if (max_peak <= 0 || max_peak <= max_val) continue;
								
								var diff = (max_val / max_peak);

								if (!equally) {
									for (var k = b; k < wnd_end; ++k) {
										var orig_val = chan_data[ k ];
										var new_val = orig_val * diff;

										var peak_diff = max_val - Math.abs (new_val);
										peak_diff *= new_val < 0 ? -ratio : ratio;

										chan_data[ k ] = Math.max (-1, Math.min (1, (new_val + peak_diff)));
									}
								}
							}
							// -----
			...
			Distortion : function ( val ) {
				return {
					filter : function ( audio_ctx, destination, source, duration ) {
						...
						{
							var curr = val[k];
							wave_shaper.curve = compute_dist ( readAutomationValue ( curr, 0.5 ) );
						}
					...
					update : function ( filter, audio_ctx, val ) {
						...
						{
							var curr = val[k];
							filter.curve = compute_dist ( readAutomationValue ( curr, 0.5 ) );
						}
						// ----
					}
				};
		...
		this.MakeSilence = MakeSilenceBuffer;
		                ...
		                this.RemoveClicks = RemoveClicks;
