(function ( w, d, PKAE ) {
	...
	function _topbarConfig ( app, ui ) {
										...
										items[i].onclick = function() {
											...
											if (window.process && window.process.versions && window.process.versions.electron && file.path) {
												...
												ipcRenderer.invoke('read-file', file.path).then(data => {
													if (data) {
														app.OpenLocalFile(data, file.name, file.path);
														q.Destroy();
													}
												}).catch(err => {
						...
						action: function () {
								new PKSimpleModal({
								  ...
								  body:'<div class="pk_row"><label for="k0">File Name</label>' + 
									'<input style="min-width:250px" placeholder="mp3 filename" value="RECwerk-output.mp3" ' +
			...
			},
			{
				...
				children:[
...

					{
						...
						{
							name:'Multitrack Workspace',
							action: function () {
								app.fireEvent ('RequestToggleMultitrackWorkspace');
							},
							setup: function ( obj ) {
								var sync = function ( val ) {
									var txt = 'Multitrack Workspace';
									if (val) obj.innerHTML = txt + ' &#10004;';
									else obj.textContent = txt;
								};
								app.listenFor ('DidToggleMultitrackWorkspace', sync);
								if (app.multitrack) sync (app.multitrack.isVisible ());
							}
						},

						{
							name:'Mixer',
							action: function () {
								app.fireEvent ('RequestToggleMultitrackMixer');
							},
							setup: function ( obj ) {
								var sync = function ( val ) {
									var txt = 'Mixer';
								...
								app.listenFor ('DidToggleMultitrackMixer', sync);
								if (app.multitrack) sync (app.multitrack.isMixerVisible ());
							}
						},
...

						{
							name:'Asset Library',
							action: function () {
								app.fireEvent ('RequestToggleMultitrackLibrary');
							},
							setup: function ( obj ) {
								var sync = function ( val ) {
									var txt = 'Asset Library';
									if (val) obj.innerHTML = txt + ' &#10004;';
									else obj.textContent = txt;
								};
								app.listenFor ('DidToggleMultitrackLibrary', sync);
								if (app.multitrack) sync (app.multitrack.isLibraryVisible ());
							}
						},

						{
							name:'Asset Library Window',
							action: function () {
			...
			},
			{
				...
				children:[
...

					{
						...
						action : function () {
							window.open ('https://github.com/flarkflarkflark/RECwerk');
						}
					}
				]
			}
		...
		function shouldIncludeMenuItem(item) {
			if (item.desktopOnly && !app.isDesktopApp) return false;
			if (item.webOnly && !app.isWebApp) return false;
			return true;
		}

		function cleanupSeparators(children) {
			var filtered = [];
			for (var i = 0; i < children.length; ++i) {
				var item = children[i];
				if (!item) continue;
				if (item.name === '---') {
					if (!filtered.length || filtered[filtered.length - 1].name === '---') continue;
				}
				filtered.push(item);
			}

			if (filtered.length && filtered[filtered.length - 1].name === '---') {
				filtered.pop();
			}
		...
		for (var i = 0; i < menu.length; ++i) {
			...
			menu[i].children = cleanupSeparators(menu[i].children.filter(shouldIncludeMenuItem));
		}
	...
	function _makeUIBarBottom ( UI, app ) {
		...
		q.SetHeight = function ( value ) {
			value = value / 1 || 130;
			q.height = value < 130 ? 130 : value;
			bar_bottom_el.style.height = q.height + 'px';
	...
	function _makeUIMainView ( UI, app ) {
		...
		UI.audioContainer = audio_container;
		UI.mainAudioView = main_audio_view;

		var wave_splitter = d.createElement ('div');
		wave_splitter.className = 'pk_wave_splitter pk_noselect';
		wave_splitter.innerHTML = '<span></span>';
		audio_container.appendChild ( wave_splitter );
		UI.waveSplitter = wave_splitter;
		...
		var wave_layout_key = 'pk_wave_height_v1';
		var wave_drag_active = false;
		var wave_drag_start_y = 0;
		var wave_drag_start_height = 0;
		var preferred_wave_height = 0;
		try {
			preferred_wave_height = (localStorage.getItem ( wave_layout_key ) || 0) / 1;
		} catch (err) {}

		function getElHeight ( class_name, fallback ) {
			var el = UI.el.getElementsByClassName ( class_name )[0];
			return el ? el.offsetHeight : (fallback || 0);
		}

		function isMultitrackVisible () {
			return !!(app.multitrack && app.multitrack.isVisible && app.multitrack.isVisible ());
		}

		function getEditorHeightBudget () {
			var header_h = getElHeight ('pk_hdr', 24);
			var toolbar_h = getElHeight ('pk_tbc', 92);
			var footer_h = footer.offsetHeight || 42;
			var dock_h = 0;

			if (UI.BarBtm && UI.BarBtm.on) {
				dock_h = UI.BarBtm.height || 0;
			}

			return Math.max (280, window.innerHeight - header_h - toolbar_h - footer_h - dock_h - 22);
		}

		function getDefaultWaveHeight () {
			var budget = getEditorHeightBudget ();
			if (!isMultitrackVisible ()) return budget;

			var ratio = (window.innerHeight < 860 ? 0.34 : 0.42);
			return Math.round (budget * ratio);
		}

		function clampWaveHeight ( value ) {
			var budget = getEditorHeightBudget ();
			var workspace_visible = isMultitrackVisible ();
			var min = workspace_visible ? 168 : Math.min (280, budget);
			var max = workspace_visible ? Math.max (min, budget - 220) : budget;
			var next = value / 1;

			if (!isFinite ( next ) || next <= 0) {
				next = getDefaultWaveHeight ();
			}
		...
		function storeWaveHeight () {
			try {
				localStorage.setItem ( wave_layout_key, String(preferred_wave_height) );
			} catch (err) {}
		}

		function applyAdaptiveLayout ( emit_resize ) {
			var width = window.innerWidth || 1280;
			var height = window.innerHeight || 720;
			var compact = (width < 1180 || height < 760);
			var wide = width >= 1720;
			var ultrawide = width >= 2460;
			var workspace_visible = isMultitrackVisible ();
			var edge_gutter = compact ? 8 : (ultrawide ? 28 : 16);
			var left_gutter = compact ? 12 : (wide ? 56 : 48);
			var shell_max_width = ultrawide ? 2520 : (wide ? 1980 : 99999);
			var wave_height = clampWaveHeight ( preferred_wave_height || getDefaultWaveHeight () );
			var editor_budget = getEditorHeightBudget ();
			var workspace_max_height = workspace_visible ? Math.max (220, editor_budget - wave_height) : 0;
			var track_list_max_height = Math.max (160, workspace_max_height - (compact ? 126 : 154));
			var library_max_height = Math.max (160, Math.round ( workspace_max_height * 0.44 ) );

			UI.el.classList.toggle ('pk_ui_compact', compact);
			UI.el.classList.toggle ('pk_ui_wide', wide);
			UI.el.classList.toggle ('pk_ui_ultrawide', ultrawide);

			UI.el.style.setProperty ('--pk-base-font-size', compact ? '12px' : (ultrawide ? '14px' : '13px'));
			UI.el.style.setProperty ('--pk-edge-gutter', edge_gutter + 'px');
			UI.el.style.setProperty ('--pk-main-left-gutter', left_gutter + 'px');
			UI.el.style.setProperty ('--pk-shell-max-width', shell_max_width + 'px');
			UI.el.style.setProperty ('--pk-wave-height', wave_height + 'px');
			UI.el.style.setProperty ('--pk-workspace-max-height', workspace_max_height + 'px');
			UI.el.style.setProperty ('--pk-track-list-max-height', track_list_max_height + 'px');
			UI.el.style.setProperty ('--pk-library-max-height', library_max_height + 'px');

			main_audio_view.style.height = wave_height + 'px';
			wave_splitter.style.display = workspace_visible ? 'flex' : 'none';

			if (emit_resize !== false) {
				app.fireEvent ('RequestResize');
			}
		}

		function stopWaveResize () {
			if (!wave_drag_active) return ;

			wave_drag_active = false;
			d.body.classList.remove ('pk_resizing_wave');
			d.removeEventListener ('mousemove', onWaveResizeMove, false);
			d.removeEventListener ('mouseup', stopWaveResize, false);
			d.removeEventListener ('mouseleave', stopWaveResize, false);
		}

		function onWaveResizeMove ( e ) {
			if (!wave_drag_active) return ;

			preferred_wave_height = clampWaveHeight ( wave_drag_start_height + (e.clientY - wave_drag_start_y) );
			storeWaveHeight ();
			applyAdaptiveLayout ( true );
		}

		wave_splitter.addEventListener ('mousedown', function ( e ) {
			if (!isMultitrackVisible ()) return ;

			e.preventDefault ();
			wave_drag_active = true;
			wave_drag_start_y = e.clientY;
			wave_drag_start_height = clampWaveHeight ( preferred_wave_height || getDefaultWaveHeight () );
			d.body.classList.add ('pk_resizing_wave');
			d.addEventListener ('mousemove', onWaveResizeMove, false);
			d.addEventListener ('mouseup', stopWaveResize, false);
			d.addEventListener ('mouseleave', stopWaveResize, false);
		}, false);

		UI.getPreferredWaveHeight = function () {
			return clampWaveHeight ( preferred_wave_height || getDefaultWaveHeight () );
		};

		UI.setPreferredWaveHeight = function ( value, emit_resize ) {
			preferred_wave_height = clampWaveHeight ( value );
			storeWaveHeight ();
			applyAdaptiveLayout ( emit_resize );
		};

		UI.syncAdaptiveLayout = function ( emit_resize ) {
			applyAdaptiveLayout ( emit_resize );
		};

		w.addEventListener ('resize', function () {
			UI.syncAdaptiveLayout ( false );
		}, false);

		app.listenFor ('DidToggleMultitrackWorkspace', function () {
			UI.syncAdaptiveLayout ( true );
		});
		...
		ttmp.innerHTML = (app.isDesktopApp ? (
			...
			'<div class="pk_empty_copy">Drop audio or a .recwerk project into the editor, or open one from your computer to start editing immediately.</div>' +
			...
			'</div>' +
			'<div class="pk_empty_meta">Drag and drop WAV, MP3, FLAC, OGG, AIFF, .recwerk and other supported files directly into the window.</div>'
		) : (
			'<div class="pk_empty_badge">Browser Build</div>' +
			'<img class="pk_empty_logo" src="img/logo.svg" alt="RECwerk" />' +
			'<div class="pk_empty_title">RECwerk in the browser</div>' +
			'<div class="pk_empty_copy">This web build is for quick waveform editing, restoration tests, and playback. Open a local audio file or .recwerk project, or drop one onto the page to see the editor in action.</div>' +
			'<div class="pk_empty_actions">' +
	...
	function _makeUIToolbar (UI) {
		...
		var makeToolbarSvg = function ( paths, view_box ) {
			return '<svg class="pk_fxicon" viewBox="' + (view_box || '0 0 24 24') + '" aria-hidden="true" focusable="false">' + paths + '</svg>';
		};
		var makeFxToolbarButton = function ( title, event_name, icon_paths, view_box ) {
			var btn = d.createElement ('button');
			btn.setAttribute ('tabIndex', -1);
			btn.className = 'pk_btn pk_fxbtn pk_inact';
			btn.innerHTML = makeToolbarSvg ( icon_paths, view_box ) + '<span>' + title + '</span>';
			btn.onclick = function () {
				if (!PKAudioEditor.engine.is_ready || this.classList.contains ('pk_inact')) {
					this.blur ();
					return ;
				}

				UI.fireEvent ( event_name );
				this.blur ();
			};
		...
		var effects = d.createElement( 'div' );
		effects.className = 'pk_effects';

		var effect_buttons = [
			makeFxToolbarButton ('Gain', 'RequestFXUI_Gain',
				'<path d="M4 15h3v5H4zM10 10h3v10h-3zM16 5h3v15h-3z"></path>'),
			makeFxToolbarButton ('Normalize', 'RequestActionFXUI_Normalize',
				'<path d="M5 18h14v2H5zM7 16V8l-2 2-1.4-1.4L8 4.2l4.4 4.4L11 10l-2-2v8zM17 6v8l2-2 1.4 1.4-4.4 4.4-4.4-4.4L13 12l2 2V6z"></path>'),
			makeFxToolbarButton ('Compressor', 'RequestActionFXUI_Compressor',
				'<path d="M4 16c2 0 2-8 4-8s2 8 4 8 2-8 4-8 2 8 4 8" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path>'),
			makeFxToolbarButton ('Paragraphic EQ', 'RequestActionFXUI_ParaGraphicEQ',
				'<path d="M5 7h2v10H5zM11 4h2v16h-2zM17 9h2v6h-2z"></path>'),
			makeFxToolbarButton ('Hard Limiter', 'RequestActionFXUI_HardLimiter',
				'<path d="M5 18h14v2H5zM7 16V8c0-2.2 1.8-4 4-4h6v2h-6c-1.1 0-2 .9-2 2v8zM17 4h2v12h-2z"></path>'),
			makeFxToolbarButton ('Click Removal', 'RequestActionFXUI_ClickRemoval',
				'<path d="M12 3l1.7 5.3H19l-4.3 3.1L16.4 17 12 13.8 7.6 17l1.7-5.6L5 8.3h5.3z"></path>'),
			makeFxToolbarButton ('Delay', 'RequestActionFXUI_Delay',
				'<path d="M7 7h10v4h3l-4 4-4-4h3V9H9v6h4v2H7z"></path>'),
			makeFxToolbarButton ('Reverb', 'RequestActionFXUI_Reverb',
				'<path d="M6 15a6 6 0 0 1 6-6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path><path d="M10 18a8 8 0 0 1 8-8" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path><circle cx="6" cy="15" r="1.5"></circle>'),
			makeFxToolbarButton ('Distortion', 'RequestActionFXUI_Distortion',
				'<path d="M4 15c2.2 0 2.2-6 4.4-6S10.6 15 12.8 15s2.2-6 4.4-6S19.4 15 21 15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path>')
		];

		for (var fx_i = 0; fx_i < effect_buttons.length; ++fx_i) {
			effects.appendChild ( effect_buttons[ fx_i ] );
		}

		UI.listenFor ('DidUnloadFile', function () {
			for (var i = 0; i < effect_buttons.length; ++i) {
				effect_buttons[i].classList.add ('pk_inact');
			}
		});
		UI.listenFor ('DidLoadFile', function () {
			for (var i = 0; i < effect_buttons.length; ++i) {
				effect_buttons[i].classList.remove ('pk_inact');
			}
		});
		...
		selection.innerHTML = '<div class="pk_sellist">' + 
			...
			'<div><span class="title">Start:</span><button type="button" tabIndex="-1" class="s_s pk_dat pk_editable">-</button></div>' + 
			'<div><span class="title">End:</span><button type="button" tabIndex="-1" class="s_e pk_dat pk_editable">-</button></div>' + 
			'<div><span  class="title">Duration:</span><button type="button" tabIndex="-1" class="s_d pk_dat pk_editable">-</button></div>' +
		'</div>';
		...
		var clampSelectionValue = function ( value, min, max ) {
			...
			(function(index) {
				sel_spans[index].onclick = function() {
					...
					if (newVal !== null && !isNaN(parseFloat(newVal))) {
						...
						var duration = PKAudioEditor.engine.wavesurfer.getDuration () || 0;
						var start = region.start || 0;
						var end = region.end || start;
						
						if (index === 0) { // Start
							newVal = clampSelectionValue (newVal, 0, duration);
							region.update({start: Math.min(newVal, end), end: Math.max(newVal, end)});
						} else if (index === 1) { // End
							newVal = clampSelectionValue (newVal, 0, duration);
							region.update({start: Math.min(start, newVal), end: Math.max(start, newVal)});
						} else if (index === 2) { // Duration
							newVal = clampSelectionValue (newVal, 0, duration);
							region.update({start: start, end: clampSelectionValue (start + newVal, start, duration)});
						}
		...
		btn_groups.appendChild ( effects );
		...
		dragNDrop( d.getElementById('app'), 'pk_overlay', function ( e, fileName, filePath ) {
			PKAudioEditor.OpenLocalFile(e, fileName, filePath);
		}, 'arrayBuffer' );
