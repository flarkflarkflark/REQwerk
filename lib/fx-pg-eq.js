		...
		var x = new PKAudioFXModal ({
			...
			title: 'New Recording',
							...
							ondestroy: function ( q ) {
								...
								app.ui.InteractionHandler.on = false;
								app.ui.KeyHandler.removeCallback (modal_esc_key);
			                    app.ui.KeyHandler.removeCallback ('recPause');
	...
	//						freqctx.drawImage (tempCanvas, 0, 0, width, 100, -(raw_pixels.toFixed(1)/1), 0, width, 100);
					...
					btn_pause.onclick = function () {
						...
						if (is_paused) {
                            ...
                            btn_pause.style.background = '#FFCC00';
                            btn_pause.style.color = '#000';
                        } else {
                            btn_pause.innerText = 'PAUSE';
                            btn_pause.style.background = '';
                            btn_pause.style.color = '';
                        }
					};

                    // Spatiebalk hotkey voor opname-pauze
                    app.ui.KeyHandler.addCallback ('recPause', function ( key, map, e ) {
                        e.preventDefault();
                        btn_pause.click();
                    }, [32]);
