(function ( w, d, PKAE ) {
	'use strict';
		...
		function getPrimaryRegion () {
			...
			if (!wavesurfer.regions || !wavesurfer.regions.list) return (null);

			if (wavesurfer.regions.list.t && wavesurfer.regions.list.t.id === 't') {
				return (wavesurfer.regions.list.t);
			}

			for (var id in wavesurfer.regions.list) {
				if (id.indexOf('click_') !== 0) {
		...
		function restorePrimaryRegion ( start, end ) {
			if (start == null || end == null) return (null);

			var region = getPrimaryRegion ();
			if (region) {
				region.update({
					start: start,
					end: end
		...
		function syncDetectedClickState ( clicks, selectionStart, selectionEnd ) {
			q.detectedClicks = clicks ? clicks.slice() : [];
			q.lastDetectedSelection = (selectionStart == null || selectionEnd == null) ? null : {
				start: selectionStart,
				end: selectionEnd
			};
		}

		function getDetectedClickState () {
			return q.detectedClicks ? q.detectedClicks.slice() : [];
		}

		this.LoadArrayBuffer = function ( e, fileName, filePath ) {
			var func = function () {
				...
				q.is_ready = false;
				q.currentFileName = fileName || "Dropped Audio";
				q.currentFilePath = filePath || "";
		...
		this.LoadDB = function ( e ) {
			var copyChannelData = function ( target, source ) {
				if (source instanceof ArrayBuffer) {
					target.set(new Float32Array(source));
					return;
				}
				...
				if (ArrayBuffer.isView(source)) {
					...
					var viewCopy = new Float32Array(source.length);
				...
				if (Array.isArray(source)) {
					...
					for (var offset = 0; offset < source.length; offset += chunkSize) {
						var end = Math.min(offset + chunkSize, source.length);
						var chunk = new Float32Array(end - offset);
						for (var i = offset, k = 0; i < end; ++i, ++k) {
							chunk[k] = source[i];
						}
				...
				throw new Error('Unsupported project channel data');
			};

			var firstChannel = e.data[0];
			var frameLength = 0;
			if (firstChannel instanceof ArrayBuffer) frameLength = firstChannel.byteLength / 4;
			else frameLength = firstChannel.length >>> 0;
		...
		this.LoadFile = function ( e ) {
			...
			var file = e.files[0];
			app.OpenLocalFile(file, file.name, file.path || '');
		};
		...
		this.LoadSample = function () {
			...
			setTimeout(function () {
				...
				app.fireEvent ('RequestZoomUI', 0);
				q.is_ready = false;
				q.currentFileName = 'test.mp3';
				wavesurfer.load ('lib/test.mp3');
			}, 180);
		}
		this.LoadURL = function ( url ) {
			app.fireEvent ('WillDownloadFile');
			q.currentFileName = url.split('/').pop() || url;
...
//			audio_el.playbackRate = 0.5;
		...
		app.listenFor ('RequestResize', function () {
			...
			var wave_height = 0;
			if (app.ui && app.ui.getPreferredWaveHeight) {
				wave_height = app.ui.getPreferredWaveHeight ();
			}
			...
			if (!wave_height) {
				...
				wave_height = (h < 280 ? 280 : h) - 168 - bottom;
			}

			wavesurfer.setHeight( Math.max (120, wave_height) );
		...
		wavesurfer.on ('ready', function () {
			...
			app.fireEvent ('DidLoadFile', q.currentFileName, q.currentFilePath);
		...
		app.listenFor ('RequestPauseToggle', function () {
			app.fireEvent ('RequestActionRecordStop');
		...
		app.listenFor ('RequestSetLoop', function () {
			console.log("RequestSetLoop triggered. Ready state:", q.is_ready);
			if (!q.is_ready) return ;

			q.loop_enabled = !q.loop_enabled;
			console.log("Loop status toggled to:", q.loop_enabled);

			var regions = wavesurfer.regions.list;
			var has_region = false;
			
			for (var id in regions) {
				regions[id].update({ loop: q.loop_enabled });
				has_region = true;
			}

			if (!has_region && q.loop_enabled)
			{
				console.log("No region found, creating full track loop");
				var region = wavesurfer.regions.add({
					start: 0.01,
			...
			app.fireEvent('DidSetLoop', q.loop_enabled);
		...
		*/
		...
		app.ui.KeyHandler.addCallback ('KeyShiftSpace' + app.id, function ( key ) {
			if (app.ui.InteractionHandler.on) return ;
			app.fireEvent ('RequestPauseToggle');
		}, [16, 32]);
		...
		(function() {
			...
			app.listenFor ('RequestLoadLocalFile', function () {
					...
					input.setAttribute ('accept', 'audio/*,.recwerk');
					...
					d.body.appendChild ( input ); // Append to body instead
...
//		});
		...
		wavesurfer.on('DidZoom', function ( e ) {
		 ...
		 wavesurfer.on('region-removed', function ( region ){
			if (region.id.indexOf('click_') === 0) return;
		...
		app.listenFor ('RequestActionFX_RemSil', function ( val ) {
			...
			handleStateInline ( start, end );
						...
						var vol_offset = 56;
							...
							for (var j = 0; j < originalBuffer.length; ++j)
							{
								var is_silent = true;
								for (var c = 0; c < channels.length; ++c)
								{
									if (Math.abs ( channels[c][j] ) >= 0.000368)
									{
										is_silent = false;
										break;
									}
		...
		app.listenFor ('RequestActionFX_PREVIEW_ClickRemoval', function ( val ) {
			...
			var region = getPrimaryRegion () || {start:0, end:wavesurfer.getDuration()};
		...
		app.listenFor ('RequestAction_DetectClicks', function ( val ) {
			if (!q.is_ready) return ;
			
			var region = getPrimaryRegion ();
			var selectionStart = region ? region.start : null;
			var selectionEnd = region ? region.end : null;
			
			var start = region ? region.start : 0;
			var duration = region ? (region.end - region.start) : wavesurfer.getDuration();

				var clicks = AudioUtils.DetectClicks( start, duration, val );
				
				syncDetectedClickState ( clicks, selectionStart, selectionEnd );
				restorePrimaryRegion ( selectionStart, selectionEnd );
				app.fireEvent ('DidCreateRegion', {
					start: selectionStart,
					end: selectionEnd
				});
				app.fireEvent('DidDetectClicksData', q.detectedClicks, q.lastDetectedSelection);
				app.fireEvent('DidDetectClicks', clicks.length);

			if (clicks.length > 0) {
				OneUp ('Detected ' + clicks.length + ' clicks');
			} else {
				OneUp ('No clicks detected');
			}
		});

		app.listenFor ('RequestAction_RemoveClicks', function ( val ) {
			if (!q.is_ready) return ;
			var region = getPrimaryRegion ();
			var selectionStart = region ? region.start : (q.lastDetectedSelection ? q.lastDetectedSelection.start : null);
			var selectionEnd = region ? region.end : (q.lastDetectedSelection ? q.lastDetectedSelection.end : null);

			var clickRegions = (val && val.clicks && val.clicks.length) ? val.clicks.slice() : getDetectedClickState ();
			
			if (clickRegions.length === 0) {
				OneUp ('No detected clicks to remove');
				return ;
			}

			// Bepaal de omvang van het gebied dat we gaan bewerken (van de eerste tot de laatste klik)
			var start = clickRegions[0].start;
			var end = clickRegions[0].end;
			clickRegions.forEach(function(r) {
				if (r.start < start) start = r.start;
				if (r.end > end) end = r.end;
			});
			
			// Voeg een kleine marge toe
			start = Math.max(0, start - 0.1);
			end = Math.min(wavesurfer.getDuration(), end + 0.1);

			app.fireEvent ('StateRequestPush', { desc : 'Remove detected clicks', meta : [ start, end - start ], data : wavesurfer.backend.buffer });
			AudioUtils.RemoveClicks( start, end - start, val, clickRegions );
			
			syncDetectedClickState ( [], null, null );
			restorePrimaryRegion ( selectionStart, selectionEnd );
			app.fireEvent ('DidCreateRegion', {
				start: selectionStart,
				end: selectionEnd
			});
			OneUp ('Removed ' + clickRegions.length + ' clicks');
		});
